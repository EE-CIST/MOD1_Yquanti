...
mod<-lm(data=sel, equip_acp~sexe+age5+etud5+statoc+log(tailmen))
...
On construit un modèle associant nos variables d'accessibilité géographiques aux variables sociales, sans ajouter les variables relatif à l'urbanisation et au statut administratif des chef-lieux pour éviter les multicolinéarités. 


```{r}


mod<-lm(data=sel, equip_acp~log_Dis_COM+log_Dis_DEP+log(tailmen)+sexe+etud5+age5+statoc)
summary(mod)
Anova(mod, type="III")
...
mod<-lm(data=sel, equip_acp~etud5+log_Dis_COM+log_Dis_COM:etud5)
summary(mod)
Anova(mod, type="III")
